...
function liquidDensity(T, comp) {
  ...
  if (dd) {
    // DIPPR 105: rho_mol = A / B^(1+(1-T/C)^D)  [kmol/m3]
...
var PU = [
  ...
  { id:"atm", lb:"atm", toP:function(v){return v*101325}, fromP:function(v){return v/101325} },
  { id:"psi", lb:"psi(a)", toP:function(v){return v*6894.757}, fromP:function(v){return v/6894.757} },
  { id:"psig", lb:"psi(g)", toP:function(v){return (v+14.696)*6894.757}, fromP:function(v){return v/6894.757-14.696} },
];
...
function bubbleP(T, x, clist) {
  ...
  for (var iter = 0; iter < 50; iter++) {
    ...
    var Zv = calcZmix(T, P, y, clist, "vapor");
    var Ke = [], s2 = 0; y = [];
    for (var ie = 0; ie < nc; ie++) { Ke.push(Math.exp(lnPhiMix_PR(ie,T,P,x,Zl,clist)-lnPhiMix_PR(ie,T,P,y.length?y:[0.5,0.5],Zv,clist))); }
    // Recalc y with new K
    Ke = []; s2 = 0;
    var Zl2 = calcZmix(T, P, x, clist, "liquid");
    var yOld = y.length > 0 ? y : x.slice();
    var Zv2 = calcZmix(T, P, yOld, clist, "vapor");
    for (var ie2 = 0; ie2 < nc; ie2++) { Ke.push(Math.exp(lnPhiMix_PR(ie2,T,P,x,Zl2,clist)-lnPhiMix_PR(ie2,T,P,yOld,Zv2,clist))); }
    y = []; s2 = 0;
...
function bubbleT(P, x, clist) {
  ...
  for (var iter=0;iter<50;iter++){
    ...
    var Ke=[],s2=0;
    for(var ie=0;ie<nc;ie++)Ke.push(Math.exp(lnPhiMix_PR(ie,T,P,x,Zl,clist)-lnPhiMix_PR(ie,T,P,y,Zv,clist)));
    ...
    var s3=0;for(var je3=0;je3<nc;je3++)s3+=x[je3]*Math.exp(lnPhiMix_PR(je3,T+dT,P,x,Zl2,clist)-lnPhiMix_PR(je3,T+dT,P,y,Zv2,clist));
    var deriv=(s3-s2)/dT;
    if(Math.abs(deriv)>1e-12){var step=-(s2-1)/deriv;if(Math.abs(step)>5)step=5*(step>0?1:-1);T=T+0.7*step}
  }
  return{T:T,y:y,K:wilsonK(T,P,clist),iter:50};
}
...
function dewT(P, y, clist) {
  ...
  for(var iter=0;iter<50;iter++){
    ...
    var dT=0.01;
    var Zl2=calcZmix(T+dT,P,x,clist,"liquid");var Zv2=calcZmix(T+dT,P,y,clist,"vapor");
    var s3=0;for(var je3=0;je3<nc;je3++)s3+=y[je3]/Math.exp(lnPhiMix_PR(je3,T+dT,P,x,Zl2,clist)-lnPhiMix_PR(je3,T+dT,P,y,Zv2,clist));
    var deriv=(s3-s2)/dT;
    if(Math.abs(deriv)>1e-12){var step=-(s2-1)/deriv;if(Math.abs(step)>5)step=5*(step>0?1:-1);T=T+0.7*step}
  }
...
// Generate T-x-y diagram: clean data with bubble and dew on same x-axis
function genTxy(P, c1, c2, nPts) {
  ...
  for (var i = 0; i <= nPts; i++) {
    ...
    var Tb = bub.T ? Math.round((bub.T-273.15)*100)/100 : null;
    var y1 = bub.y ? Math.round(bub.y[0]*1000)/1000 : x1;
    data.push({ x1: Math.round(x1*1000)/1000, Tbub: Tb, y1: y1 });
  }
  // Build dew data mapped to y1 axis
  var dewData = [];
  for (var j = 0; j <= nPts; j++) {
    var yy1 = j / nPts;
    var dew = dewT(P, [yy1, 1-yy1], clist);
    var Td = dew.T ? Math.round((dew.T-273.15)*100)/100 : null;
    dewData.push({ y1: Math.round(yy1*1000)/1000, Tdew: Td });
  }
  return { bub: data, dew: dewData };
}
...
function genPxy(T, c1, c2, nPts) {
  ...
  for (var i = 0; i <= nPts; i++) {
    ...
    bubData.push({ x1: Math.round(x1*1000)/1000, Pbub: bub.P ? Math.round(bub.P/1e6*10000)/10000 : null, y1: bub.y ? Math.round(bub.y[0]*1000)/1000 : x1 });
  }
  for (var j = 0; j <= nPts; j++) {
    var yy1 = j / nPts;
    var dew = dewP(T, [yy1, 1-yy1], clist);
    dewData.push({ y1: Math.round(yy1*1000)/1000, Pdew: dew.P ? Math.round(dew.P/1e6*10000)/10000 : null });
  }
  return { bub: bubData, dew: dewData };
}
...
function calcPipe(L, D, eps, vel, rho, mu) {
  ...
  return {Re:Re, f:f, dP:dP, dH:dH, regime:Re<2300?"Laminar":"Turbulent"};
}
...
function steamPHflash(P, H_target) {
  // H_target in kJ/kg, P in Pa. Find T such that h(T,P) = H_target
  // Start with estimate from ideal gas: T ~ H/Cp + offset
  var T = 400; // initial K
  for (var iter = 0; iter < 100; iter++) {
    var r = steamR2(T, P);
    if (!r || !r.h) { T += 10; continue; }
    var diff = r.h - H_target; // kJ/kg
    if (Math.abs(diff) < 0.001) return {T:T, props:r, iter:iter+1};
    // Numerical derivative dh/dT
    var r2 = steamR2(T + 0.1, P);
    var dhdT = (r2.h - r.h) / 0.1;
    if (Math.abs(dhdT) < 1e-10) dhdT = 1;
    var step = -diff / dhdT;
    if (Math.abs(step) > 50) step = 50 * (step > 0 ? 1 : -1);
    T = T + step;
    if (T < 273.15) T = 280;
    if (T > 1073) T = 1000;
  }
  return {T:T, props:steamR2(T,P), iter:100, warn:"max iter"};
}

function steamPSflash(P, S_target) {
  // S_target in kJ/(kg*K), P in Pa. Find T such that s(T,P) = S_target
  var T = 500;
  for (var iter = 0; iter < 100; iter++) {
    var r = steamR2(T, P);
    if (!r || !r.s) { T += 10; continue; }
    var diff = r.s - S_target;
    if (Math.abs(diff) < 0.0001) return {T:T, props:r, iter:iter+1};
    var r2 = steamR2(T + 0.1, P);
    var dsdT = (r2.s - r.s) / 0.1;
...
function calcSVgas(W,T,Z,MW,g,P1,Pb,Kd){Kd=Kd||0.975;var Pc=P1*Math.pow(2/(g+1),g/(g-1));var cr=Pb<=Pc;var Cv=apiC(g);var Kb=1;if(!cr){var r=Pb/P1;Kb=Math.sqrt(g/(g-1)*Math.pow(r,2/g)*(1-Math.pow(r,(g-1)/g))/(1-r))/(Cv/520)*Math.sqrt(2/g)}var A=13160*W*Math.sqrt(T*Z)/(Cv*Kd*P1*Kb*Math.sqrt(MW));var orf=[["D",71],["E",126],["F",198],["G",325],["H",506],["J",830],["K",1186],["L",1841],["M",2323],["N",2800],["P",4116],["Q",6452],["R",10323],["T",16774]];var rec=null;for(var i=0;i<orf.length;i++){if(orf[i][1]>=A){rec=orf[i];break}}return{A:A,cr:cr,C:Cv,Kb:Kb,Pc:Pc,rec:rec}}
...
function calcCvLiq(Q_m3h, G, P1_kPa, P2_kPa, Ff, Fl) {
  ...
  return {Cv: Cv, dPeff: dPeff, choked: choked, regime: choked ? "Choked" : "Normal"};
}
function calcCvGas(W_kgh, P1_kPa, P2_kPa, T_K, MW, gamma, Z) {
  ...
  return {Cv: Cv, x: x, xeff: xeff, Y: Y, Fk: Fk, choked: choked, regime: choked ? "Choked" : "Normal"};
}
...
var TX = {
  subtitle:{cn:"\u5316\u5de5\u70ed\u529b\u5b66\u8ba1\u7b97\u5e73\u53f0",en:"Chemical Thermodynamics Calculator",bi:"Chem. Thermo. Calculator"},
...
function buildRows(res, lg) {
  ...
  if (res.isVLE) {
    ...
    if (tp === "flash") { addR(lg==="en"?"Vapor Fraction":"\u6c14\u76f8\u5206\u7387 VF", res.beta, "--"); addR("T", res.T ? res.T-273.15 : "", "\u00b0C"); addR("P", res.P ? res.P/1e6 : "", "MPa"); }
    if (tp === "bubP") { addR(lg==="en"?"Bubble P":"\u6ce1\u70b9\u538b\u529b", res.P/1e6, "MPa"); addR("T", res.T-273.15, "\u00b0C"); }
    if (tp === "bubT") { addR(lg==="en"?"Bubble T":"\u6ce1\u70b9\u6e29\u5ea6", res.T-273.15, "\u00b0C"); addR("P", res.P/1e6, "MPa"); }
    if (tp === "dewP") { addR(lg==="en"?"Dew P":"\u9732\u70b9\u538b\u529b", res.P/1e6, "MPa"); addR("T", res.T-273.15, "\u00b0C"); }
    if (tp === "dewT") { addR(lg==="en"?"Dew T":"\u9732\u70b9\u6e29\u5ea6", res.T-273.15, "\u00b0C"); addR("P", res.P/1e6, "MPa"); }
    if (res.iter) addR(lg==="en"?"Iterations":"\u8fed\u4ee3", res.iter, "");
    addR("--- K-values ---","","",true);
    for (var ik=0;ik<(res.clist||[]).length;ik++) addR("K("+res.clist[ik].f+")", res.K?res.K[ik]:"", "");
    if (res.x&&(tp==="flash"||tp==="dewP"||tp==="dewT")) { addR("--- "+(lg==="en"?"Liquid xi":"\u6db2\u76f8 xi")+" ---","","",true); for(var ix=0;ix<res.x.length;ix++) addR(res.clist[ix].f,res.x[ix],""); }
    if (res.y&&(tp==="flash"||tp==="bubP"||tp==="bubT")) { addR("--- "+(lg==="en"?"Vapor yi":"\u6c14\u76f8 yi")+" ---","","",true); for(var iy=0;iy<res.y.length;iy++) addR(res.clist[iy].f,res.y[iy],""); }
    return r;
  }
...
export default function App() {
  ...
  useEffect(function(){
    try { localStorage.setItem("ctp_inputs", JSON.stringify({tv:tv,tu:tu,pv:pv,pu:pu,lang:lang,mode:mode,group:group,meth:meth})); } catch(e){}
  },[tv,tu,pv,pu,lang,mode,group,meth]);
  // Load on mount
  useEffect(function(){
    try {
      var stored=localStorage.getItem("ctp_inputs");
      if(stored){var d=JSON.parse(stored);if(d.tv)setTv(d.tv);if(d.tu)setTu0(d.tu);if(d.pv)setPv(d.pv);if(d.pu)setPu0(d.pu);if(d.lang)setLang(d.lang);if(d.mode)setMode(d.mode);if(d.group)setGroup(d.group);if(d.meth)setMeth(d.meth);}
    } catch(e){}
  },[]);
  ...
  var doCalc = useCallback(function(){
    ...
    if (mode==="vle") {
      ...
      if(Math.abs(zS-1)>0.01){setErr("Sum zi = 1");return;}
      ...
      var vr;
      if(vleType==="flash"){vr=tpFlash(TK,PPa,z,clist);vr.type="flash";vr.T=TK;vr.P=PPa;}
      ...
      vr.isVLE=true;vr.clist=clist;vr.z=z;setRes(vr);return;
    }
    if (mode==="diag") {
      ...
      if(vleType==="txy"||vleType==="flash"){var dd=genTxy(PPa,d1,d2,30);setDiagData({type:"txy",bub:dd.bub,dew:dd.dew,P:PPa,c1:comps[0],c2:comps[1]});}
      else{var dd2=genPxy(TK,d1,d2,30);setDiagData({type:"pxy",bub:dd2.bub,dew:dd2.dew,T:TK,c1:comps[0],c2:comps[1]});}
      return;
    }
    var c0b=comps[0];
  ...
  else if(mode==="vle"){showT=vleType==="flash"||vleType==="bubP"||vleType==="dewP";showP=vleType==="flash"||vleType==="bubT"||vleType==="dewT";}
  ...
  return (
    <div style={{minHeight:"100vh",backgroundColor:C.bg,color:C.text,fontFamily:"-apple-system,sans-serif"}}>
      ...
      <div style={{background:"linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #334155 100%)",padding:"18px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,boxShadow:"0 2px 12px rgba(8,145,178,0.25)"}}>
        <div>
          <div style={{fontSize:22,fontWeight:700,color:"#ffffff",letterSpacing:0.5}}>ChemThermo</div>
      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"16px 12px"}}>
        ...
        <div style={{display:"flex",gap:14,flexWrap:"wrap",alignItems:"flex-start"}}>
            ...
            {(mode==="vle"||mode==="diag")?(
              <div style={cardStyle}>
                <div style={{fontSize:11,color:C.textL,marginBottom:8,fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>
                  {mode==="vle"?"VLE Type":"Diagram Type"}
                </div>
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  {(mode==="vle"?[{id:"flash",lb:"TP Flash"},{id:"bubP",lb:"Bubble P"},{id:"bubT",lb:"Bubble T"},{id:"dewP",lb:"Dew P"},{id:"dewT",lb:"Dew T"}]:[{id:"txy",lb:"T-x-y"},{id:"pxy",lb:"P-x-y"}]).map(function(v){return (
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                {showH?(<div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <input type="number" value={hval} onChange={function(e){setHval(e.target.value)}} placeholder="kJ/kg" style={inputStyle} />
                    <span style={{color:C.textL,fontSize:11}}>kJ/kg</span>
                  </div>
                  <div style={{fontSize:10,color:C.textL,marginTop:4}}>{lang==="en"?"Enthalpy (superheated steam ~2700-3500)":"\u7113\u503c (\u8fc7\u70ed\u84b8\u6c7d\u7ea6 2700-3500)"}</div>
                </div>):null}
                {showS?(<div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{color:C.textL,fontSize:12,minWidth:32,fontWeight:600}}>S</span>
                    <input type="number" value={sval} onChange={function(e){setSval(e.target.value)}} placeholder="kJ/(kg*K)" style={inputStyle} />
                    <span style={{color:C.textL,fontSize:11}}>kJ/(kg*K)</span>
                  </div>
                  <div style={{fontSize:10,color:C.textL,marginTop:4}}>{lang==="en"?"Entropy (superheated steam ~6.5-8.5)":"\u71b5\u503c (\u8fc7\u70ed\u84b8\u6c7d\u7ea6 6.5-8.5)"}</div>
                </div>):null}
              </div>
          ...
          <div style={{flex:group==="props"?"1 1 480px":"1 1 100%",minWidth:300}}>
            ...
            {diagData?(<div style={{backgroundColor:C.white,borderRadius:C.radius,padding:14,marginTop:res?12:0,boxShadow:C.shadow,border:"1px solid "+C.border}}>
              ...
              <div style={{fontSize:11,color:C.textL,marginBottom:10}}>{diagData.c1.f+" (1) + "+diagData.c2.f+" (2)"}</div>
            ...
            {mode==="comp"?(function(){
              ...
              var doComp = function(){
                ...
                setErr(null);
                var r=calcCompressor(P1,T1,P2,eta,gam,mw,fl>0?fl/3600:0);
              ...
              return (<div style={{backgroundColor:C.white,borderRadius:C.radius,padding:16,boxShadow:C.shadow,border:"1px solid "+C.border}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {svType==="gas"?(<div style={{display:"flex",flexDirection:"column",gap:8}}>
                    ...
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>MW</span><input type="number" value={svIn.mw} onChange={function(e){setSvI("mw",e.target.value)}} style={iS} /></div>
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>Cp/Cv</span><input type="number" value={svIn.gamma} onChange={function(e){setSvI("gamma",e.target.value)}} style={iS} /></div>
                  </div>):(<div style={{display:"flex",flexDirection:"column",gap:8}}>
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>Q(m3/h)</span><input type="number" value={svIn.q} onChange={function(e){setSvI("q",e.target.value)}} style={iS} /></div>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>P1(kPa)</span><input type="number" value={svIn.p1} onChange={function(e){setSvI("p1",e.target.value)}} style={iS} /></div>
            ...
            {mode==="gasflow"?(function(){
              var iS={flex:1,padding:"10px 12px",backgroundColor:C.white,border:"1.5px solid "+C.border,borderRadius:C.radius,color:C.text,fontSize:14,fontFamily:"monospace",outline:"none",boxSizing:"border-box"};
              var lS={color:C.textL,fontSize:11,minWidth:90,fontWeight:600};
              var TK=parseFloat(gfIn.t)+273.15;var PP=parseFloat(gfIn.p)*1e6;var Qa=parseFloat(gfIn.qa);
              var nm=isNaN(Qa)?0:gasStdFlow(Qa,TK,PP,273.15,101325);
              var sm=isNaN(Qa)?0:gasStdFlow(Qa,TK,PP,288.15,101325);
              var us=isNaN(Qa)?0:gasStdFlow(Qa,TK,PP,288.71,101325);
              var scfh=us*35.3147;var scfm=scfh/60;var mmscfd=scfh*24/1e6;
              return (<div style={{backgroundColor:C.white,borderRadius:C.radius,padding:16,boxShadow:C.shadow,border:"1px solid "+C.border}}>
                <div style={{fontSize:14,fontWeight:700,color:C.pri,marginBottom:4}}>{lang==="en"?"Standard Gas Flow":"\u6807\u51c6\u6c14\u4f53\u6d41\u91cf"}</div>
                <div style={{fontSize:10,color:C.textL,marginBottom:10}}>{lang==="en"?"Ideal gas law: Q_std = Q_act * (P/P_std) * (T_std/T_act)":"\u7406\u60f3\u6c14\u4f53: Q\u6807 = Q\u5b9e * (P/P\u6807) * (T\u6807/T\u5b9e)"}</div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>{lang==="en"?"Actual m3/h":"\u5b9e\u9645m3/h"}</span><input type="number" value={gfIn.qa} onChange={function(e){setGf("qa",e.target.value)}} style={iS} /></div>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>T(C)</span><input type="number" value={gfIn.t} onChange={function(e){setGf("t",e.target.value)}} style={iS} /></div>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={lS}>P(MPa abs)</span><input type="number" value={gfIn.p} onChange={function(e){setGf("p",e.target.value)}} style={iS} /></div>
                </div>
                <div style={{marginTop:14,backgroundColor:C.bg,borderRadius:8,padding:12}}>
                  <div style={{display:"flex",flexDirection:"column",gap:6,fontFamily:"monospace",fontSize:13}}>
                    <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.textM}}>Nm3/h <span style={{fontSize:10,color:C.textL}}>(0C, 101.325kPa)</span></span><span style={{color:C.pri,fontWeight:700}}>{nm.toFixed(2)}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.textM}}>Sm3/h <span style={{fontSize:10,color:C.textL}}>(15C, 101.325kPa)</span></span><span style={{color:C.pri,fontWeight:700}}>{sm.toFixed(2)}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.textM}}>SCFM <span style={{fontSize:10,color:C.textL}}>(60F, 14.696psi)</span></span><span style={{color:C.pri,fontWeight:700}}>{scfm.toFixed(2)}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.textM}}>SCFH <span style={{fontSize:10,color:C.textL}}>(60F, 14.696psi)</span></span><span style={{color:C.pri,fontWeight:700}}>{scfh.toFixed(1)}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.textM}}>MMSCFD <span style={{fontSize:10,color:C.textL}}>(60F, 14.696psi)</span></span><span style={{color:C.pri,fontWeight:700}}>{mmscfd.toFixed(6)}</span></div>
                  </div>
                </div>
              </div>);
            })():null}
        ...
        {showInfo?(<div onClick={function(){setShowInfo(false)}} style={{position:"fixed",top:0,left:0,right:0,bottom:0,backgroundColor:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div onClick={function(e){e.stopPropagation()}} style={{backgroundColor:C.white,borderRadius:12,padding:24,maxWidth:540,width:"100%",maxHeight:"80vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
            <div style={{fontSize:20,fontWeight:700,color:C.pri,marginBottom:4}}>ChemThermo</div>
            <div style={{fontSize:12,color:C.textM,marginBottom:16}}>{lang==="en"?"Chemical Engineering Calculation Platform":"\u5316\u5de5\u5de5\u7a0b\u8ba1\u7b97\u5e73\u53f0"}</div>
            <div style={{fontSize:11,color:C.text,lineHeight:1.9}}>
              ...
              <div style={{fontWeight:700,color:C.cProp,marginTop:8,fontSize:12}}>{"VLE "}{lang==="en"?"Phase Equilibria":"\u76f8\u5e73\u8861"}{" & "}{lang==="en"?"Phase Diagrams":"\u76f8\u56fe"}</div>
              <div style={{color:C.textM}}>{lang==="en"?"TP Flash / Bubble P,T / Dew P,T (bisection + EOS) | T-x-y, P-x-y binary diagrams | P-T phase diagram: H2O, CO2 (triple/critical point, vaporization/sublimation/melting lines)":"TP\u95ea\u84b8/\u6ce1\u70b9P,T/\u9732\u70b9P,T(\u4e8c\u5206+EOS) | T-x-y/P-x-y\u4e8c\u5143\u76f8\u56fe | P-T\u4e09\u76f8\u56fe: H2O/CO2(\u4e09\u76f8\u70b9/\u4e34\u754c\u70b9/\u6c7d\u5316\u7ebf/\u5347\u534e\u7ebf/\u7194\u5316\u7ebf)"}</div>
              <div style={{fontWeight:700,color:C.cEquip,marginTop:10,fontSize:12}}>{lang==="en"?"EQUIPMENT - 7 Modules":"\u8bbe\u5907\u8ba1\u7b97 - 7\u4e2a\u6a21\u5757"}</div>
              <div style={{color:C.textM}}>{lang==="en"?"Compressor/Expander (isentropic) | Pump (hydraulic power) | Pipe dP (Darcy-Weisbach + 10 fittings K-values) | Refrig. COP (vapor compression) | LMTD (counter/parallel) | Insulation heat loss (with common k-values)":"\u538b\u7f29\u673a/\u81a8\u80c0\u673a(\u7b49\u71b5) | \u6cf5(\u6db2\u529b\u529f\u7387) | \u7ba1\u8def\u538b\u964d(Darcy-Weisbach+10\u79cd\u7ba1\u4ef6K\u503c) | \u5236\u51b7COP(\u84b8\u6c14\u538b\u7f29) | LMTD\u6362\u70ed | \u4fdd\u6e29\u6563\u70ed(\u542b\u5e38\u7528k\u503c)"}</div>
              <div style={{fontWeight:700,color:C.cTool,marginTop:10,fontSize:12}}>{lang==="en"?"TOOLS - 9 Modules":"\u5de5\u7a0b\u5de5\u5177 - 9\u4e2a\u6a21\u5757"}</div>
        ...
        <div style={{marginTop:24,padding:"16px 0",borderTop:"1px solid "+C.border,textAlign:"center"}}>
          <div style={{color:C.textL,fontSize:10,marginBottom:6}}>
            ChemThermo v5.0 | 36 Components | PR/SRK/IAPWS-IF97 | ASME B31.3 | API 520 | ISO 5167
          </div>
          <div style={{color:C.textL,fontSize:10}}>
            {lang==="en"?"Feedback: ":"\u53cd\u9988: "}
